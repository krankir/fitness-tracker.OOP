...
class Running(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        - self.COEFF_CALORIE_2) * self.weight / self.M_IN_KM * (self.duration * self.DUR_MIN)
        return calories           
...
class SportsWalking(Training):
    ...
    def get_spent_calories(self) -> float:
        """Получить количество затраченных калорий."""
        calories = (self.COEFF_CALORIE_1 * self.weight + (self.get_mean_speed()**2 // self.height ) \
            * self.COEFF_CALORIE_2 * self.weight) * (self.duration * self.DUR_MIN)
        return calories   
